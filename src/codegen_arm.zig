...
pub const CodeBuffer = struct {
    ...
    pub fn addShift(self: *CodeBuffer, rd: u8, rn: u8, rm: u8, shift: u8) void {
        self.dword(0x8B000000 | (@as(u32, shift) << 10) | (@as(u32, rm) << 16) | (@as(u32, rn) << 5) | rd);
    }

    pub fn subShift(self: *CodeBuffer, rd: u8, rn: u8, rm: u8, shift: u8) void {
        self.dword(0xCB000000 | (@as(u32, shift) << 10) | (@as(u32, rm) << 16) | (@as(u32, rn) << 5) | rd);
    }
    ...
    pub fn buildElf64(self: *CodeBuffer) void {
        ...
        i = 0; while (i < 8) : (i += 1) out[24 + i] = le64(base + code_foff)[i];
        ...
        i = 0; while (i < 4) : (i += 1) out[phdr_off + 0 + i] = le32(1)[i];     // p_type = PT_LOAD
        i = 0; while (i < 4) : (i += 1) out[phdr_off + 4 + i] = le32(5)[i];     // p_flags = R+X
        i = 0; while (i < 8) : (i += 1) out[phdr_off + 8 + i] = le64(0)[i];     // p_offset = 0 (covers headers too)
        i = 0; while (i < 8) : (i += 1) out[phdr_off + 16 + i] = le64(base)[i]; // p_vaddr
        i = 0; while (i < 8) : (i += 1) out[phdr_off + 24 + i] = le64(base)[i]; // p_paddr
        i = 0; while (i < 8) : (i += 1) out[phdr_off + 32 + i] = le64(total)[i]; // p_filesz
        i = 0; while (i < 8) : (i += 1) out[phdr_off + 40 + i] = le64(total)[i]; // p_memsz
        i = 0; while (i < 8) : (i += 1) out[phdr_off + 48 + i] = le64(0x1000)[i]; // p_align
