...
pub const Framebuffer = struct {
    ...
    pub fn drawGlyphScaled(self: *Framebuffer, glyph: [FONT_H]u16, x: i32, y: i32, val: u32, scale: u32) void {
        var gy: u32 = 0;
        while (gy < FONT_H) : (gy += 1) {
            var gx: u32 = 0;
            while (gx < FONT_W) : (gx += 1) {
                if ((glyph[gy] & (@as(u16, 1) << @intCast(FONT_W - 1 - gx))) != 0) {
        ...
        while (i < text.len) {
            ...
            if (b0 & 0x80 != 0) {
                if ((b0 & 0xE0) == 0xC0 and i + 1 < text.len) {
                    cp = (@as(u32, b0 & 0x1F) << 6) | @as(u32, text[i + 1] & 0x3F);
                    step = 2;
                } else if ((b0 & 0xF0) == 0xE0 and i + 2 < text.len) {
                    cp = (@as(u32, b0 & 0x0F) << 12) | (@as(u32, text[i + 1] & 0x3F) << 6) | @as(u32, text[i + 2] & 0x3F);
                    step = 3;
                } else if ((b0 & 0xF8) == 0xF0 and i + 3 < text.len) {
                    cp = (@as(u32, b0 & 0x07) << 18) | (@as(u32, text[i + 1] & 0x3F) << 12) | (@as(u32, text[i + 2] & 0x3F) << 6) | @as(u32, text[i + 3] & 0x3F);
                    step = 4;
                }
            }
            i += step;
            const gidx = font.codepointIndex(cp);
            const glyph_off = gidx * glyph_h;
            var gy: u32 = 0;
            while (gy < glyph_h) : (gy += 1) {
                ...
                while (gx < glyph_w) : (gx += 1) {
                    if ((row_data & (@as(u16, 1) << @intCast(glyph_w - 1 - gx))) != 0) {
...
pub fn getGlyph(cp: u32) [font.FONT_H]u16 {
    const idx = font.codepointIndex(cp);
    const off = idx * font.FONT_H;
    if (off + font.FONT_H <= font.fontData.len) {
        var result: [font.FONT_H]u16 = .{0} ** font.FONT_H;
        for (0..font.FONT_H) |i| {
            result[i] = font.fontData[off + i];
        }
        return result;
    }
    return .{0} ** font.FONT_H;
}

pub const FONT_W = font.FONT_W;
pub const FONT_H = font.FONT_H;
